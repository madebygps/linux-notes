# Intro to AKS

## Learn more

- [Docker overview](https://docs.docker.com/get-started/overview/)

- [Introduction to Docker containers](https://aka.ms/dockcontainers5) 

- [Build a containerized web application with Docker.](https://aka.ms/introcontainers5)

- [Administer containers in Azure](https://aka.ms/admincontainers5)

- [Introduction to AKS](https://aka.ms/dockcontainers5)

- [Deploy an AKS Cluster with Azure CLI](https://aka.ms/febwalkthrukube7)

- [Deploy a containerized application on AKS](https://aka.ms/febdeploycontainers7)


